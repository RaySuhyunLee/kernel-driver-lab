...
static long long read_tsc(void)
{
	unsigned eax, edx;
	long long result;
	__asm__ __volatile("rdtsc" : "=a"(eax), "=d"(edx));
	result = eax | (unsigned long long)edx << 0x20;
...
static long msrdrv_ioctl(struct file *f, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	for (i=0; i<= MSR_VEC_LIMIT; i++, msrops++) {
		switch (msrops->op) {
			case MSR_NOP:
				dprintk(KERN_INFO "Module " DEV_NAME ": seen MSR_NOP command\n")
				break;
			case MSR_STOP:
				dprintk(KERN_INFO "Module " DEV_NAME ": seen MSR_STOP command\n")
				goto label_end;
			case MSR_READ:
			...
			case MSR_WRITE:
				...
				break;
			case MSR_RDTSC:
				dprintk(KERN_INFO "Module " DEV_NAME ": seen MSR_RDTSC command\n")
				msrops->value = read_tsc();
				break;
			default:
				dprintk(KERN_INFO "Module " DEV_NAME ": Unknown option 0x%x\n", msrops->op)
				return 1;
		}
	}
label_end:
